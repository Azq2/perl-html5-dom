...
L<HTML5::DOM|https://metacpan.org/pod/HTML5::DOM> is a fast HTML5 parser and DOM manipulatin library with CSS4 selector, fully conformant with the HTML5 specification.
...
Get specificity in array C<[b, a, c]> (ordered by weight)
...
Detect text encoding. Single method for both L<detectRussian|/detectRussian> and L<detectUnicode|/detectUnicode>.
...
(such as L<HTML5::DOM::parse|/parse>, L<HTML5::DOM::parseChunkEnd|/parseChunkEnd>, L<HTML5::DOM::Tree::parseFragment|/parseFragment>)
waiting for parsing done.

If async 1, you must manualy call C<$tree-E<gt>wait> and C<$node-E<gt>wait> for waiting parsing done.

Or use non-blocking C<$tree-E<gt>parsed> and C<$node-E<gt>parsed> to determine parsing state.

This options works only if C<threads> > 0
