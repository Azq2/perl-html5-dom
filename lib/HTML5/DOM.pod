...
C<$async->wait> returns L<HTML5::DOM::AsyncResult|/"HTML5::DOM::AsyncResult"> object.
...
C<$async->parsed> returns C<1> if parsing done. Else returns C<0>.

C<$async->tree> returns L<HTML5::DOM::Tree|/"HTML5::DOM::Tree"> object if parsing done. Else returns C<undef>.

3. Intergation with L<HTML::MyHTML|https://metacpan.org/pod/EV>
